...
def get_msg(key: str, lang: str = "ru", **kwargs) -> str:
    """
    Return localized message for key. Default language 'ru'.
    Supports .format(**kwargs).
    """
    cfg = load_json()
    val = cfg.get(key)
    if isinstance(val, dict):
        # try requested lang, fallback to ru, then first available
...
def build_main_menu(lang: str = "ru") -> InlineKeyboardMarkup:
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=get_msg("menu_completed_orders", lang) if False else get_msg("Выполненые заказы", lang) if get_msg("Выполненые заказы", lang) else get_msg("completed_orders_text", lang), callback_data="completed_orders")],  # noqa: E501
        [InlineKeyboardButton(text=get_msg("Пригласить друга", lang) if get_msg("Пригласить друга", lang) else get_msg("invite_intro", lang), callback_data="invite_friend")],  # noqa: E501
        [InlineKeyboardButton(text=get_msg("Промоакции", lang) if get_msg("Промоакции", lang) else get_msg("promotions_header", lang), callback_data="promotions")],
        [InlineKeyboardButton(text=get_msg("Вывод средств", lang) if get_msg("Вывод средств", lang) else get_msg("withdraw_enter_amount", lang), callback_data="withdraw")],
    ])
    return kb

def build_invite_friend_menu(lang: str = "ru"):
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text=get_msg("back_to_start", lang), callback_data="to_start")],
            [InlineKeyboardButton(text=get_msg("contact_manager_button", lang), callback_data="contact_manager")],
        ]
    )
...
def build_promo_list(promos: List[Dict]) -> InlineKeyboardMarkup:
    ...
    if not promos:
        kb.add(InlineKeyboardButton(text=get_msg("back_to_start", "ru"), callback_data="to_start"))
        return kb
